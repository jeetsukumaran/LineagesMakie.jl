...
(2) Update the document, the PRD, etc. to cleanly reflect the vocabulary that we decided here. In each, add a note that we shall be using the controlled vocabulary file at `STYLE-vocabulary.md` which remains the authoritative list. In BOTH, do provide a summary list of key prefererd terms, brief definition, and proscribed by understood alternates: vertex, edge, seedvertex, length, height, depth, ages etc
